...
foreach ($item in $windowsOnly) {
    ...
    $assignUri = "https://graph.microsoft.com/beta/deviceManagement/$base/$($item.Id)/assignments"
    $postUri   = "https://graph.microsoft.com/beta/deviceManagement/$base/$($item.Id)/assign"

    try {
        # 1. Get current assignments
        $existing = (Invoke-MgGraphRequest -Uri $assignUri -Method GET -OutputType PSObject).value

        # 2. Check if our group already exists
        $exists = $false
        foreach ($a in $existing) {
            if ($a.target.groupId -eq $GroupId) { $exists = $true; break }
        }

        if (-not $exists) {
            # 3. Append our new target
            ...
            # 4. Re-POST full merged list
            $body = @{ assignments = $assignments } | ConvertTo-Json -Depth 5
            Invoke-MgGraphRequest -Uri $postUri -Method POST -Body $body -ContentType "application/json"
            Write-Host "✔ Added group to $($item.DisplayName) [$($item.Source)]" -ForegroundColor Cyan
        }
        else {
            Write-Host "ℹ Group already assigned to $($item.DisplayName)" -ForegroundColor DarkGray
        }
    }
    catch {
        Write-Warning "✖ Failed on $($item.DisplayName): $($_.Exception.Message)"
    }
}
